...
interface UseCustomerTypingIndicatorReturn {
...
export function useCustomerTypingIndicator(
  conversationId: string,
  apiKey: string
): UseCustomerTypingIndicatorReturn {
  ...
  // Send typing indicator to API (for customer side)
  const sendTypingIndicator = useCallback(async (typing: boolean) => {
    try {
      // For customer side, we'll use a different endpoint or broadcast directly
      ...
      if (typing) {
        await channel.send({
          ...
          payload: {
            conversationId,
            userId: "customer", // For customer side, we use a generic ID
            userType: "customer",
      ...
      console.error("Failed to send customer typing indicator:", error);
    }
