...
import { HubSpotConnectionValidator } from "@/lib/integrations/hubspot-connection-validator";
  ...
  try {
    ...
    const rateLimitResult = await rateLimit(request, {
      ...
      keyGenerator: async (req) => {
        ...
        return `hubspot-validate:${session?.user?.id || "anonymous"}`;
      },
    });
    ...
    const integration = await oauth2Manager.getIntegration(
      session.user.id,
      "hubspot"
    );
    if (!integration) {
    ...
    const hubspotClient = new HubSpotApiClient(integration);
    const validator = new HubSpotConnectionValidator(
      hubspotClient,
      oauth2Manager
    );
...
export async function GET(request: NextRequest) {
  try {
    ...
    const lastCheck = integration.lastHealthCheck
      ? new Date(integration.lastHealthCheck)
      : null;
    ...
    return NextResponse.json({
      ...
      requiresAuth:
        healthStatus === "ERROR" &&
        integration.config?.connectionDetails === undefined,
    });
  } catch (error) {
    console.error("Error getting HubSpot validation status:", error);
    return NextResponse.json(
      {
        success: false,
