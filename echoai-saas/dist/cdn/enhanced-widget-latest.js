 ...
 * Built: 2025-09-11T18:42:38.171Z
 * License: MIT
 * 
...
(function () {
  ...
  class APIClient {
    ...
    async initializeSession() {
      const response = await this.request("/api/chat/session", {
        ...
        body: JSON.stringify({
          chatbotId: this.config.chatbotId,
          externalUserEmail: "widget-user@example.com",
        }),
      });
    ...
    async loadConversationHistory(page = 1, limit = 20, search = '') {
      ...
      const response = await this.request(`http://localhost:3000/api/chat/history?${params.toString()}`);
      return response.json();
    }
    ...
    async escalate(conversationId, reason) {
      return this.request("/api/escalation", {
        method: "POST",
        body: JSON.stringify({
  ...
  // Theme Manager - Handle dynamic theming and CSS custom properties
  class ThemeManager {
    constructor(config) {
      this.config = config;
    ...
    initializeTheme() {
      ...
      this.setCSSVariable(
        ...
        this.config.primaryColor || "#3b82f6"
      );
      this.setCSSVariable(
        "--echoai-primary-hover",
        this.lightenColor(this.config.primaryColor || "#3b82f6", -10)
      );
      this.setCSSVariable(
        "--echoai-primary-light",
        this.lightenColor(this.config.primaryColor || "#3b82f6", 90)
      );
      ...
      this.detectAndApplyColorScheme();
    }
    ...
    applyDarkMode() {
      this.setCSSVariable("--echoai-text-primary", "#f9fafb");
      this.setCSSVariable("--echoai-text-secondary", "#d1d5db");
      this.setCSSVariable("--echoai-text-muted", "#9ca3af");
      this.setCSSVariable("--echoai-background", "#1f2937");
      this.setCSSVariable("--echoai-background-secondary", "#374151");
      this.setCSSVariable("--echoai-border", "#4b5563");
      this.setCSSVariable("--echoai-border-light", "#6b7280");
    }
  ...
  class UIManager {
      ...
      this.templates.faq = {
        item: (faq) => `
          <div class="echoai-faq-item" 
            ...
            <div class="echoai-faq-question">
              ...
              ${
                faq.popularity > 0
                  ? `<span class="echoai-faq-popularity" title="Popular FAQ">🔥</span>`
                  : ""
              }
            </div>
            ...
            <div class="echoai-faq-meta">
              ...
              ${
                faq.helpfulCount > 0
                  ? `<span class="echoai-faq-helpful">👍 ${faq.helpfulCount}</span>`
                  : ""
              }
            </div>
          </div>
        ...
        empty: () => `
          <div class="echoai-no-content">
            <div class="echoai-no-content-icon">❓</div>
        ...
        error: () => `
          <div class="echoai-error">
            <div class="echoai-error-icon">⚠️</div>
    ...
    applyTheme() {
      ...
      this.container.classList.add(
        `echoai-theme-${this.config.theme || "auto"}`
      );
      ...
      return `
        <div class="echoai-tab-panel echoai-tab-panel-transition" data-panel="faq" style="display: none;" role="tabpanel" aria-labelledby="faq-tab" id="faq-panel" tabindex="0">
          <div class="echoai-faq-container">
            <div class="echoai-faq-header">
              <h2 class="echoai-faq-title">Frequently Asked Questions</h2>
              <p class="echoai-faq-subtitle">Find quick answers to common questions</p>
            </div>
            <div class="echoai-faq-search">
              <div class="echoai-search-input-container">
                <input type="text" 
                       ...
                       autocomplete="off" />
                <div class="echoai-search-icon">🔍</div>
              </div>
            </div>
      ...
      return `
        <div class="echoai-input-container">
          ...
          <div class="echoai-input-row">
            ${this.config.enableImageUpload ? this.generateUploadButton() : ""}
    ...
    generateEscalationButton() {
      return `
        <button class="echoai-escalate-btn" id="echoai-escalate-btn" title="Talk to human" aria-label="Talk to human">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    generateBranding() {
      return this.config.showBranding
        ? '<div class="echoai-branding">Powered by EchoAI</div>'
        : "";
    }
    ...
    async loadHistoryContent(page = 1, search = '') {
      try {
        const historyResponse = await this.apiClient.loadConversationHistory(page, 20, search);
        
        // Handle both array response and paginated response
        const historyData = Array.isArray(historyResponse) ? historyResponse : historyResponse.conversations || historyResponse.data || [];
        ...
        const processedHistory = historyData.map(item => ({
          ...
          preview: this.truncateText(item.lastMessage || item.preview || 'No messages', 60),
          messageCount: item.messageCount || item.messages?.length || 0,
          timestamp: this.formatTimestamp(item.updatedAt || item.createdAt || item.timestamp),
        ...
        if (!historyResponse.pagination) {
          processedHistory.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        }

        // Update state - append if loading more pages, replace if first page
        const currentHistory = this.stateManager.getState().conversationHistory || [];
        const updatedHistory = page === 1 ? processedHistory : [...currentHistory, ...processedHistory];
    ...
    updateHistoryPanelContent(panel, content) {
      ...
      const paginationContainer = panel.querySelector("#echoai-history-pagination");
    ...
    updateHistoryPagination(panel, pagination) {
      ...
      const loadMoreBtn = paginationContainer.querySelector(".echoai-load-more-btn");
      const paginationText = paginationContainer.querySelector(".echoai-pagination-text");
        ...
        if (loadMoreBtn) {
          ...
          this.eventManager.addEventListener(newBtn, "click", async () => {
            ...
            try {
              ...
              const content = await this.loadHistoryContent(nextPage, searchTerm);
              this.updateHistoryPanelContent(panel, content);
            } catch (error) {
              console.error("EchoAI: Failed to load more history:", error);
              newBtn.disabled = false;
              newBtn.textContent = "Load More Conversations";
              this.showNotification("Failed to load more conversations", "error");
            }
          });
        }
      } else {
      ...
      if (paginationText) {
        ...
        const currentCount = this.stateManager.getState().conversationHistory?.length || 0;
    ...
    filterHistory(searchTerm) {
      const historyPanel = this.container.querySelector('[data-panel="history"]');
      if (!historyPanel) return;

      const historyItems = historyPanel.querySelectorAll(".echoai-history-item");
      const term = searchTerm.toLowerCase().trim();

      let visibleCount = 0;

      historyItems.forEach((item) => {
        const preview = item.querySelector(".echoai-history-preview")?.textContent?.toLowerCase() || '';
        const timestamp = item.querySelector(".echoai-history-meta")?.textContent?.toLowerCase() || '';
        
        // Search in both preview and timestamp
        const matches = !term || preview.includes(term) || timestamp.includes(term);
    ...
    highlightSearchTerm(item, term) {
      const previewElement = item.querySelector(".echoai-history-preview");
      if (!previewElement) return;

      const originalText = previewElement.textContent;
      const regex = new RegExp(`(${this.escapeRegex(term)})`, 'gi');
      const highlightedText = originalText.replace(regex, '<mark class="echoai-search-highlight">$1</mark>');
    ...
    removeHighlights(item) {
      ...
      const marks = previewElement.querySelectorAll('.echoai-search-highlight');
      marks.forEach(mark => {
    ...
    updateHistoryEmptyState(panel, visibleCount, searchTerm) {
      ...
      const existingEmptyState = panel.querySelector(".echoai-search-no-results");
    ...
    createMessageElement(message) {
      ...
      messageDiv.className = `echoai-message echoai-message-${message.role}`;
      ...
      // Add status class
      if (message.status) {
        messageDiv.classList.add(`echoai-message-${message.status}`);
      }
      ...
      statusDiv.className = "echoai-message-status";
      statusDiv.innerHTML = this.getStatusIcon(message.status);
      statusDiv.setAttribute("aria-label", `Message ${message.status}`);
    ...
    // Create message actions (copy, feedback, etc.)
    createMessageActions(message) {
      ...
      if (message.role === "assistant" || message.role === "agent") {
        ...
        thumbsUpBtn.className = "echoai-message-action echoai-thumbs-up";
        ...
        thumbsDownBtn.className = "echoai-message-action echoai-thumbs-down";
        thumbsDownBtn.setAttribute("aria-label", "Not helpful");
        thumbsDownBtn.innerHTML = `
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M17 14V2l-4 4-5-4v12l4.5 4.5L17 14z"></path>
            <path d="M3.16 19.39a5.5 5.5 0 0 0 7.78 0L12 18.33l1.06 1.06a5.5 5.5 0 0 0 7.78-7.78l-1.06-1.06L12 2.77 4.22 10.55l-1.06 1.06a5.5 5.5 0 0 0 0 7.78z"></path>
          </svg>
        `;
    ...
    updateTheme(theme) {
      ...
      // Add new theme class
      this.container.classList.add(`echoai-theme-${theme}`);

      // Update theme manager
      this.config.theme = theme;
      this.themeManager.config.theme = theme;
      this.themeManager.detectAndApplyColorScheme();
    }
  ...
  window.EchoAI = {
    init: function (config) {
      ...
      this.config = {
        ...
        streamingConfig: {
          enabled: config.streamingConfig?.enabled !== false,
      ...
      const sendMessage = async (messageContent) => {
        ...
        try {
          ...
          if (this.config.streamingConfig.enabled) {
            try {
              await this.sendStreamingMessage(message, imageUrl);
    ...
    handleEscalation: function (reason) {
      const currentStatus = this.getConversationStatus();

      // Don't escalate if already with human agent or resolved
      if (currentStatus === "AWAITING_HUMAN_RESPONSE") {
        this.uiManager.showNotification(
          "You are already connected to a human agent",
          "info",
          3000
        );
        return;
      ...
      if (currentStatus === "RESOLVED") {
        this.uiManager.showNotification(
          "This conversation has been resolved. Please start a new conversation for further assistance.",
          "info",
          4000
        );
        return;
      ...
      this.showEscalationDialog(reason);
    },
    ...
    loadConversation: async function (sessionId) {
      if (!sessionId) {
        console.error("EchoAI: No session ID provided for conversation loading");
        return;
      }
      ...
      try {
        ...
        if (conversation.metadata) {
          this.stateManager.setState({
            ...
            conversationActions: conversation.metadata.conversationActions || [],
          });
        }
    ...
    setupRealtimeEventListeners: function () {
        ...
        if (this.uiManager) {
          switch (data.status) {
            case "AWAITING_HUMAN_RESPONSE":
              this.uiManager.showNotification(
                `${
                  data.data.agentName || "Human agent"
                } has joined the conversation`,
                "info",
                4000
              );
            ...
            case "AI_HANDLING":
              this.uiManager.showNotification(
                "AI assistant is now handling your conversation",
                "info",
                3000
              );
              break;
      ...
      this.eventManager.on("message-delivery-failed", (data) => {
        ...
        if (this.uiManager) {
          this.uiManager.showNotification(
            "Message delivery failed. Please check your connection.",
            "error",
            5000
          );
        }
    ...
    initializeSession: async function () {
      ...
      try {
        const sessionId = await this.apiClient.initializeSession();
    ...
    showIntelligencePanel: function (enhancedData) {
      ...
      if (panelContent.trim()) {
        intelligencePanel.innerHTML = panelContent;
    ...
    loadFAQs: async function () {
      ...
      try {
        ...
        if (faqs.length === 0) {
          faqList.innerHTML =
            '<div class="echoai-no-content">No FAQs available</div>';
          return;
        }
        ...
        faqList.innerHTML = faqs
          .map(
            (faq) => `
          <div class="echoai-faq-item" onclick="window.EchoAI.selectFAQ('${faq.question.replace(
            /'/g,
            "\\'"
          )}', '${faq.answer.replace(/'/g, "\\'")}')">
            <div class="echoai-faq-question">${faq.question}</div>
            <div class="echoai-faq-answer">${faq.answer}</div>
          </div>
        `
          )
          .join("");
        ...
        if (searchInput) {
          this.eventManager.addEventListener(searchInput, "input", (e) => {
            ...
            faqItems.forEach((item) => {
              const question = item
                .querySelector(".echoai-faq-question")
                .textContent.toLowerCase();
              const answer = item
                .querySelector(".echoai-faq-answer")
                .textContent.toLowerCase();
        ...
        faqList.innerHTML =
          '<div class="echoai-error">Failed to load FAQs</div>';
      }
    },
      ...
      try {
        ...
        if (history.length === 0) {
          historyList.innerHTML =
            '<div class="echoai-no-content">No conversation history</div>';
          return;
        }
        ...
        historyList.innerHTML = history
          .map(
            (item) => `
          <div class="echoai-history-item" onclick="window.EchoAI.loadConversation('${
            item.sessionId
          }')">
            <div class="echoai-history-preview">${item.preview}</div>
            <div class="echoai-history-meta">
        ...
        if (searchInput) {
          this.eventManager.addEventListener(searchInput, "input", (e) => {
            const query = e.target.value.toLowerCase();
            const historyItems = this.container.querySelectorAll(
              ".echoai-history-item"
            );

            historyItems.forEach((item) => {
              const preview = item
                .querySelector(".echoai-history-preview")
                .textContent.toLowerCase();
        ...
        console.error("EchoAI: Failed to load conversation history:", error);
        historyList.innerHTML =
          '<div class="echoai-error">Failed to load history</div>';
      }
    },
    ...
    getStyles: function () {
      return `
        ...
        :root {
          --echoai-primary-color: #3b82f6;
          --echoai-primary-hover: #2563eb;
          --echoai-primary-light: #eff6ff;
          --echoai-text-primary: #374151;
          --echoai-text-secondary: #6b7280;
          --echoai-text-muted: #9ca3af;
          --echoai-background: #ffffff;
          --echoai-background-secondary: #f8fafc;
          --echoai-border: #e5e7eb;
          --echoai-border-light: #f3f4f6;
          --echoai-shadow: 0 20px 60px rgba(0, 0, 0, 0.15);
          --echoai-radius: 16px;
          --echoai-radius-small: 8px;
          --echoai-spacing-xs: 4px;
          --echoai-spacing-sm: 8px;
          --echoai-spacing-md: 12px;
          --echoai-spacing-lg: 16px;
          --echoai-spacing-xl: 20px;
          --echoai-font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
        ...
        .echoai-enhanced-widget-container * {
          ...
          font-family: var(--echoai-font-family);
        }
        ...
        .echoai-enhanced-widget {
          width: 380px;
          height: 600px;
          background: var(--echoai-background);
          border-radius: var(--echoai-radius);
          box-shadow: var(--echoai-shadow);
          display: flex;
          flex-direction: column;
          overflow: hidden;
          margin-bottom: var(--echoai-spacing-sm);
        ...
        /* Header styles */
        .echoai-header {
          background: var(--echoai-primary-color);
          color: white;
          padding: var(--echoai-spacing-xl);
          display: flex;
          align-items: center;
          justify-content: space-between;
          border-radius: var(--echoai-radius) var(--echoai-radius) 0 0;
        }
        
        .echoai-header-content {
          display: flex;
          align-items: center;
          gap: var(--echoai-spacing-md);
        }
        ...
        .echoai-connection-disconnected .echoai-connection-indicator {
          color: #fbbf24;
        }

        .echoai-connection-disconnected .echoai-connection-indicator svg {
        ...
        .echoai-human-agent-indicator {
          ...
          color: white;
          padding: var(--echoai-spacing-md) var(--echoai-spacing-xl);
          display: flex;
          align-items: center;
          gap: var(--echoai-spacing-md);
        ...
        .echoai-agent-indicator-visible {
          transform: translateY(0);
          opacity: 1;
        }

        .echoai-agent-indicator-hiding {
          transform: translateY(-100%);
          opacity: 0;
        }
        ...
        .echoai-status-dot {
          ...
          background: #10b981;
          border-radius: 50%;
          animation: echoai-status-pulse 2s infinite;
        }

        @keyframes echoai-status-pulse {
          0%, 100% { 
            opacity: 1;
            transform: scale(1);
          }
        ...
        .echoai-close-btn {
          ...
          color: white;
          cursor: pointer;
          padding: var(--echoai-spacing-sm);
          border-radius: var(--echoai-spacing-sm);
          transition: var(--echoai-transition);
          display: flex;
          align-items: center;
          justify-content: center;
        ...
        .echoai-tabs {
          display: flex;
          background: var(--echoai-background-secondary);
          border-bottom: 1px solid var(--echoai-border);
        }
        
        .echoai-tab {
          flex: 1;
          padding: var(--echoai-spacing-md) var(--echoai-spacing-lg);
          background: none;
          border: none;
          cursor: pointer;
          font-size: 14px;
          font-weight: 500;
          color: var(--echoai-text-secondary);
          transition: var(--echoai-transition);
          position: relative;
        }
        
        .echoai-tab:hover {
          background: var(--echoai-border);
          color: var(--echoai-text-primary);
        }

        .echoai-tab:focus {
          outline: 2px solid var(--echoai-primary-color);
          outline-offset: -2px;
        }
        ...
        /* Enhanced Message Display System Styles */
        .echoai-message {
          display: flex;
          flex-direction: column;
          max-width: 85%;
          ...
          position: relative;
          margin-bottom: var(--echoai-spacing-md);
        }
        ...
        .echoai-message-content {
          background: var(--echoai-background);
          padding: var(--echoai-spacing-md) var(--echoai-spacing-lg);
          border-radius: var(--echoai-radius-small);
        ...
        .echoai-message-agent .echoai-message-content {
          background: #f0f9ff;
          color: var(--echoai-text-primary);
        ...
        .echoai-message-text a {
          color: inherit;
          text-decoration: underline;
        }

        .echoai-message-user .echoai-message-text a {
          color: rgba(255, 255, 255, 0.9);
        }
        ...
        .echoai-message-time {
          ...
          margin-top: var(--echoai-spacing-xs);
          text-align: right;
        }

        .echoai-message-user .echoai-message-time {
          text-align: left;
        }
        ...
        .echoai-message-user .echoai-message-status {
          right: auto;
          left: 0;
        }

        .echoai-message-status svg {
          width: 12px;
          height: 12px;
        }

        .echoai-message-delivered .echoai-message-status {
          color: #10b981;
        }

        .echoai-message-pending .echoai-message-status {
          color: #f59e0b;
        }

        .echoai-message-failed .echoai-message-status {
          color: #ef4444;
        }
        ...
        .echoai-message-user .echoai-message-actions {
          right: auto;
          left: -40px;
        }

        .echoai-message:hover .echoai-message-actions {
          opacity: 1;
        }
        ...
        .echoai-message-action:hover {
          background: var(--echoai-primary-color);
          color: white;
          border-color: var(--echoai-primary-color);
          transform: scale(1.1);
        }

        .echoai-message-action:focus {
          outline: 2px solid var(--echoai-primary-color);
          outline-offset: 2px;
        }

        .echoai-message-action svg {
          width: 14px;
          height: 14px;
        }

        .echoai-message-feedback {
          display: flex;
          flex-direction: column;
          gap: var(--echoai-spacing-xs);
        }

        .echoai-feedback-btn.echoai-feedback-active {
          background: var(--echoai-primary-color);
          color: white;
          border-color: var(--echoai-primary-color);
        }
        ...
        .echoai-streaming .echoai-message-content {
          background: var(--echoai-background);
          border: 1px solid var(--echoai-border);
          position: relative;
          animation: echoai-streaming-pulse 2s infinite ease-in-out;
        }
        ...
        .echoai-streaming-cursor {
          ...
          margin-left: 2px;
          color: var(--echoai-primary-color);
          font-weight: bold;
        ...
        @keyframes echoai-streaming-pulse {
          0%, 100% { 
        ...
        .echoai-streaming-controls {
          ...
          animation: echoai-slide-up 0.3s ease-out;
        }
        ...
        .echoai-cancel-streaming-btn {
          ...
          align-items: center;
          gap: var(--echoai-spacing-xs);
          padding: var(--echoai-spacing-sm) var(--echoai-spacing-md);
          background: #ef4444;
          color: white;
          border: none;
          border-radius: var(--echoai-radius-small);
        ...
        .echoai-cancel-streaming-btn:focus {
          outline: 2px solid rgba(239, 68, 68, 0.5);
          outline-offset: 2px;
        }

        .echoai-cancel-streaming-btn:active {
          transform: translateY(0);
        }

        .echoai-cancel-streaming-btn svg {
          width: 14px;
          height: 14px;
        }

        @keyframes echoai-slide-up {
          from {
            opacity: 0;
        ...
        .echoai-typing .echoai-message-content {
          background: var(--echoai-background);
          border: 1px solid var(--echoai-border);
          padding: var(--echoai-spacing-lg);
        }
        ...
        .echoai-typing-indicator span:nth-child(1) {
          animation-delay: -0.32s;
        }

        .echoai-typing-indicator span:nth-child(2) {
        ...
        .echoai-messages.echoai-virtual-scrolling {
          contain: layout style paint;
        }

        .echoai-virtual-spacer {
          flex-shrink: 0;
        }
        ...
        @media (max-width: 480px) {
          ...
          .echoai-message-actions {
            ...
            margin-top: var(--echoai-spacing-sm);
            gap: var(--echoai-spacing-sm);
          }
        ...
        .echoai-message-time {
          ...
          color: var(--echoai-text-muted);
          margin-top: var(--echoai-spacing-xs);
          padding: 0 var(--echoai-spacing-xs);
        }

        /* Streaming and typing indicators */
        .echoai-streaming .echoai-cursor {
        ...
        .echoai-typing-indicator {
          ...
          gap: var(--echoai-spacing-xs);
          align-items: center;
          padding: var(--echoai-spacing-xs) 0;
        }
        ...
        /* Intelligence panel */
        .echoai-intelligence-panel {
          padding: var(--echoai-spacing-lg) var(--echoai-spacing-xl);
        ...
        .echoai-intelligence-section {
          margin-bottom: var(--echoai-spacing-lg);
        }
        ...
        .echoai-intelligence-section h4 {
          ...
          font-weight: 600;
          color: var(--echoai-text-secondary);
          margin: 0 0 var(--echoai-spacing-sm) 0;
        ...
        .echoai-action-btn {
          ...
          background: var(--echoai-background);
          border: 1px solid var(--echoai-border);
          border-radius: var(--echoai-spacing-md);
          font-size: 12px;
          cursor: pointer;
          transition: var(--echoai-transition);
          color: var(--echoai-text-primary);
        ...
        .echoai-proactive-btn:focus,
        .echoai-topic-btn:focus,
        .echoai-action-btn:focus {
          outline: 2px solid var(--echoai-primary-color);
          outline-offset: 2px;
        }

        .echoai-confidence-score {
          font-size: 10px;
        ...
        .echoai-action-btn:hover .echoai-confidence-score {
          background: rgba(255, 255, 255, 0.2);
          color: white;
        }

        /* Intelligence Metrics */
        .echoai-intelligence-metrics {
          display: flex;
          flex-wrap: wrap;
          gap: var(--echoai-spacing-sm);
        }
        ...
        .echoai-metric {
          ...
          background: var(--echoai-background);
          border: 1px solid var(--echoai-border);
          padding: var(--echoai-spacing-xs) var(--echoai-spacing-sm);
          border-radius: var(--echoai-spacing-sm);
          color: var(--echoai-text-secondary);
        }

        .echoai-debug-section {
          opacity: 0.8;
          font-size: 11px;
        }
        ...
        .echoai-escalation-suggestion {
          ...
          border-radius: var(--echoai-radius-small);
          margin: var(--echoai-spacing-md) var(--echoai-spacing-xl);
          overflow: hidden;
          transition: all 0.3s ease;
        }

        .echoai-suggestion-content {
          display: flex;
          align-items: center;
          padding: var(--echoai-spacing-md);
          gap: var(--echoai-spacing-md);
        }

        .echoai-suggestion-icon {
          flex-shrink: 0;
          width: 40px;
          height: 40px;
        ...
        .echoai-suggestion-text {
          flex: 1;
          min-width: 0;
        }

        .echoai-suggestion-title {
          font-weight: 600;
          color: #92400e;
          margin-bottom: var(--echoai-spacing-xs);
        }

        .echoai-suggestion-reason {
          font-size: 12px;
          color: #a16207;
        }

        .echoai-suggestion-actions {
          display: flex;
          gap: var(--echoai-spacing-sm);
          flex-shrink: 0;
        }

        .echoai-suggestion-btn {
          padding: var(--echoai-spacing-sm) var(--echoai-spacing-md);
          border: none;
          border-radius: var(--echoai-spacing-sm);
          font-size: 12px;
        ...
        .echoai-suggestion-accept:hover {
          background: #b45309;
          transform: translateY(-1px);
        }
        ...
        .echoai-suggestion-dismiss:hover {
          background: rgba(217, 119, 6, 0.1);
        }

        /* Lead Collection Dialog */
        .echoai-lead-collection-dialog .echoai-dialog-content {
          max-width: 400px;
        }
        ...
        .echoai-form-group input:focus {
          outline: none;
          border-color: var(--echoai-primary-color);
          box-shadow: 0 0 0 3px var(--echoai-primary-light);
        }

        .echoai-form-group input:invalid {
          border-color: #ef4444;
        }
        ...
        .echoai-dialog-visible {
          opacity: 1;
        }

        .echoai-dialog-hiding {
          opacity: 0;
        }

        .echoai-dialog-overlay {
          position: absolute;
          top: 0;
          left: 0;
          right: 0;
        ...
        .echoai-dialog-visible .echoai-dialog-content {
          transform: scale(1);
        }
        ...
        .echoai-dialog-body p {
          margin: 0 0 var(--echoai-spacing-md) 0;
          color: var(--echoai-text-secondary);
          line-height: 1.5;
        }
        ...
        .echoai-btn {
          padding: var(--echoai-spacing-sm) var(--echoai-spacing-lg);
          border: none;
          border-radius: var(--echoai-spacing-sm);
          font-size: 14px;
          font-weight: 500;
        ...
        .echoai-btn-secondary:hover {
          background: var(--echoai-background-secondary);
          border-color: var(--echoai-text-secondary);
        }

        /* Responsive Design for Intelligence Features */
        @media (max-width: 480px) {
          .echoai-intelligence-panel {
            padding: var(--echoai-spacing-md);
          }
        ...
        .echoai-remove-image {
          position: absolute;
          top: var(--echoai-spacing-xs);
          right: var(--echoai-spacing-xs);
          background: rgba(0, 0, 0, 0.5);
          color: white;
          border: none;
          border-radius: 50%;
        ...
        .echoai-image-preview-container {
          display: flex;
          align-items: center;
          gap: var(--echoai-spacing-md);
        }

        .echoai-preview-image {
          max-width: 100px;
          max-height: 100px;
          border-radius: var(--echoai-radius-small);
          object-fit: cover;
        ...
        .echoai-progress-bar {
          width: 100%;
          height: 4px;
          background: var(--echoai-border-light);
          border-radius: 2px;
          overflow: hidden;
          margin-bottom: var(--echoai-spacing-xs);
        }

        .echoai-progress-fill {
          height: 100%;
          background: var(--echoai-primary-color);
        ...
        .echoai-progress-text {
          font-size: 12px;
          color: var(--echoai-text-muted);
        }

        .echoai-drag-drop-area {
          position: absolute;
          top: 0;
          left: 0;
          right: 0;
          bottom: 0;
          background: rgba(59, 130, 246, 0.1);
          border: 2px dashed var(--echoai-primary-color);
          border-radius: var(--echoai-radius);
          display: flex;
          align-items: center;
          justify-content: center;
        ...
        .echoai-drag-drop-content {
          text-align: center;
          padding: var(--echoai-spacing-xl);
        }

        .echoai-drag-drop-content svg {
          color: var(--echoai-primary-color);
          margin-bottom: var(--echoai-spacing-md);
        }

        .echoai-drag-drop-content p {
          margin: 0 0 var(--echoai-spacing-xs) 0;
          color: var(--echoai-text-primary);
          font-weight: 500;
        }
        ...
        .echoai-browse-btn:hover {
          color: var(--echoai-primary-hover);
        }

        .echoai-drag-drop-content small {
          color: var(--echoai-text-muted);
          font-size: 12px;
        }
        ...
        .echoai-upload-btn:focus,
        .echoai-escalate-btn:focus {
          outline: 2px solid var(--echoai-primary-color);
          outline-offset: 2px;
        }
        ...
        .echoai-send-btn:focus {
          outline: 2px solid var(--echoai-primary-light);
          outline-offset: 2px;
        }
        ...
        .echoai-toggle-btn:focus {
          outline: 3px solid var(--echoai-primary-light);
          outline-offset: 3px;
        }

        /* FAQ and History containers */
        ...
        .echoai-history-list {
          flex: 1;
          overflow-y: auto;
          padding: 0 var(--echoai-spacing-xl) var(--echoai-spacing-xl);
        }
        ...
        .echoai-faq-category-section {
          margin-bottom: var(--echoai-spacing-xl);
        }

        .echoai-faq-category-title {
          margin: 0 0 var(--echoai-spacing-md) 0;
          padding: var(--echoai-spacing-sm) 0;
          font-size: 16px;
          font-weight: 600;
          color: var(--echoai-primary-color);
          border-bottom: 2px solid var(--echoai-primary-light);
          position: sticky;
          top: 0;
          background: var(--echoai-background);
          z-index: 1;
        ...
        .echoai-faq-meta {
          ...
          align-items: center;
          margin-top: var(--echoai-spacing-sm);
          padding-top: var(--echoai-spacing-sm);
          border-top: 1px solid var(--echoai-border-light);
          font-size: 12px;
        }
        ...
        .echoai-faq-category {
          background: var(--echoai-primary-light);
          color: var(--echoai-primary-color);
          padding: 2px var(--echoai-spacing-sm);
          border-radius: var(--echoai-spacing-xs);
          font-weight: 500;
        }

        .echoai-faq-popularity {
          margin-left: var(--echoai-spacing-sm);
          font-size: 14px;
        }
        ...
        .echoai-faq-search-results {
          padding: var(--echoai-spacing-md) 0;
        }

        .echoai-faq-search-header {
          padding: var(--echoai-spacing-md) 0;
          font-size: 14px;
          color: var(--echoai-text-secondary);
          border-bottom: 1px solid var(--echoai-border-light);
          margin-bottom: var(--echoai-spacing-md);
        }

        .echoai-faq-item-hover {
          background: var(--echoai-background-secondary) !important;
          border-color: var(--echoai-primary-color) !important;
        }

        .echoai-error-retry {
          margin-top: var(--echoai-spacing-md);
          padding: var(--echoai-spacing-sm) var(--echoai-spacing-md);
          background: var(--echoai-primary-color);
          color: white;
          border: none;
        ...
        .echoai-error-retry:hover {
          background: var(--echoai-primary-dark);
        }
        ...
        .echoai-load-more-btn {
          ...
          color: white;
          border: none;
          border-radius: var(--echoai-border-radius);
          cursor: pointer;
          font-size: 14px;
          font-weight: 500;
          transition: all 0.2s ease;
        ...
        .echoai-load-more-btn:hover:not(:disabled) {
          background: var(--echoai-primary-dark);
          transform: translateY(-1px);
        }

        .echoai-load-more-btn:disabled {
          opacity: 0.6;
          cursor: not-allowed;
          transform: none;
        }

        .echoai-pagination-info {
          margin-top: var(--echoai-spacing-sm);
          text-align: center;
        }
        ...
        .echoai-clear-search:hover {
          background: var(--echoai-primary-dark);
        }

        /* History item states */
        .echoai-history-item-loading {
          opacity: 0.6;
          pointer-events: none;
        }
